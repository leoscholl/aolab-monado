...
{
	explicit android_surface_callbacks(struct xrt_instance_android *xinst_android) : instance_android(xinst_android)
	{}
	struct xrt_instance_android *instance_android;
	...
	try {
		return asc->callback_collection.invokeCallbacks(
		    event, [=](enum xrt_android_surface_event event, xrt_android_surface_event_handler_t callback,
		               void *userdata) { return callback(asc->instance_android, window, event, userdata); });
	}
	CATCH_CLAUSES("invoking callbacks", -1)
}

struct android_surface_callbacks *
android_surface_callbacks_create(struct xrt_instance_android *xinst_android)
{
	try {
		auto ret = std::make_unique<android_surface_callbacks>(xinst_android);
